...
contains
  ...
  subroutine Broadcast_NAO2GTO()
    ...
    do isp=1,nspecies
      ...
      call MPI_Bcast(spp%orbnl_contract, maxn_orbnl, MPI_integer, 0, &
&       MPI_Comm_World, MPIerror)

      ! Index of spherical harmonic GTOs
      call MPI_Bcast(spp%orbnl_index_sphi, maxn_orbnl, MPI_integer, 0, &
&       MPI_Comm_World, MPIerror)
...
&       MPI_double_precision, 0, MPI_Comm_World, MPIerror)
      call MPI_Bcast(spp%orbnl_coefficient(1,1), maxn_contract*maxn_orbnl, &
&       MPI_double_precision, 0, MPI_Comm_World, MPIerror)

      ! Other variables
      call MPI_Bcast(spp%sphi(1,1), ncon_max*maxnorbs, MPI_double_precision, &
&       0, MPI_Comm_World, MPIerror)
      call MPI_Bcast(spp%pgf_radius(1,1), maxn_contract*maxn_orbnl, &
&       MPI_double_precision, 0, MPI_Comm_World, MPIerror)
      call MPI_Bcast(spp%shell_radius(1), maxn_orbnl, MPI_double_precision, &
&       0, MPI_Comm_World, MPIerror)
      call MPI_Bcast(spp%kind_radius, 1, MPI_double_precision, 0, &
&       MPI_Comm_World, MPIerror)
