...
contains
  ...
  subroutine siesta_forces(istep)
    ...
    use siesta_geom,   only: isa, na_s, na_u
    ...
    use atomlist,      only: iaorb, indxua, iphorb, &
&                            no_l, no_s, no_u, lasto, Qtot
...
&            isa, listh, listhptr, nsc, numh, ucell, xa)
           call nao2gto_libint_init(hfx_libint, l_max, hfx_system)
         end if
