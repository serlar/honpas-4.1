...
! *****************************************************************************
  FUNCTION exp_radius(l,alpha,threshold,prefactor,epsin) RESULT(radius)
    ...
    !write(96,*) l,alpha, threshold, prefactor.epsin
    ...
    IF (l < 0) THEN
       WRITE(6,*) "The angular momentum quantum number is negative"
       STOP
    END IF

    IF (alpha == 0.0_dp) THEN
       WRITE(6,*) "The Gaussian function exponent is zero"
       STOP
    ELSE
       a = ABS(alpha)
    END IF
    ...
    IF (threshold == 0.0_dp) THEN
       WRITE(6,*) "The requested threshold is zero"
       STOP
    ELSE
       t = ABS(threshold)
    END IF
...
! *****************************************************************************
  FUNCTION exp_radius_very_extended(la_min, la_max, lb_min, lb_max, &
&     pab, o1, o2, ra, rb, rp, zetp, eps, prefactor, cutoff, epsin) &
&     RESULT(radius)
    ...
    ! Get the local prefactor, we'll now use the largest density matrix
    ! element of the block to screen
...
! *****************************************************************************
  subroutine create_md_ftable(nmax, tmin, tmax, tdelta)
    ...
    if ( current_nmax > -1 ) then
      write(6,*) "Error: an incomplete gamma function table is already allocated"
      stop
    end if

    ! Check arguments
    if ( nmax < 0 ) then
      write(6,*) "Error: a negative n value for the initialization of &
...
&        (tdelta <= 0.0_dp) .or. &
&        (tmin > tmax) ) then
      write(6,*) 'Error: invalid arguments'
      stop
    end if
    ...
    if ( istat /= 0 ) then
      write(6,*) 'err: memory allocate, ftable'
      stop
    end if
    ftable(:,:) = 0.0_dp
